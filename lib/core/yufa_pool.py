...
from lib.settings import rootPath,Ent_O_Top_yufakey,Ent_O_Top_yufa,Ent_O_Top_source
from lib.clasetting import addToClipboard
import threading
class Yufa_pool():
    def __init__(self, gui):
        ...
        self.tree.heading("index", text="序号", command=lambda :self.treeview_sort_column(self.tree, 'index', False))
    ...
    def search(self):
        ...
        with sqlite3.connect(rootPath+'/lib/data/cms_finger.db') as conn:
            cursor = conn.cursor()
            result = cursor.execute("SELECT name, keys FROM `tide` WHERE name LIKE '%{}%' LIMIT 100".format(yufakey))
            index = 1
            for row in result:
                ...
                index += 1
            result = cursor.execute("SELECT name, keys FROM `fofa_back` WHERE name LIKE '%{}%' LIMIT 100".format(yufakey))
            for row in result:
                self.tree.insert("","end",values=(
                            index,
                            row[0],
