...
def check(**kwargs):
    ...
    if encode is not None:
        ...
        print("[-] Badchars: " + str(badchars))
        print("[*] Payload: " + str(encode))
        try:
            for _ in range(number):
                result = XORpass().encode(encode, charset=charset, badchars=badchars)
                print("[#] Encoded Payload: " + result)
        except Exception as ex:
            print("Error encoding the payload: ", ex)

    if decode is not None:
        print("[*] Encoded Payload: " + str(decode))
        result = XORpass().decode(decode)
