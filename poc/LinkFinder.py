...
#获取单个JS里的请求接口信息并将结果输出到linkfinder.html
...
def html_save(html):
    ...
    try:
        ...
        s = Template(template)
        text_file = open('./lib/html/'+args.output, "wb")
...
class Args:
    def __init__(self, domain, input, output='linkfinder.html', regex=None, burp=False, cookies='', timeout=5):
        self.domain = domain
        self.input = input
        self.output = output
        self.regex = regex
