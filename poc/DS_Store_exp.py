        ...
        while True:
            ...
            try:
                ...
                if response.status_code == 200:
                    self.folder_name = './lib/result/DS_Store/'+netloc.replace(':', '_') + '/'.join(path.split('/')[:-1])
                    if not os.path.exists(self.folder_name):
                        os.makedirs(self.folder_name)
                    with open('./lib/result/DS_Store/'+netloc.replace(':', '_') + path, 'wb') as outFile:
                        self.lock.acquire()
...
def check(**kwargs):
    ...
    s = Scanner(url)
    s.scan()
    print('[+]打开 %s 查看结果'%(os.getcwd()+'\\lib\\result\\DS_Store'))
